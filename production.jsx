  ...
  return (
    <div>
      ...
      <div style={{padding:18}}>
        <StageSection step="1" title="Architecture" sub="line → stage → machine tree · the slowest stage caps the line"><ProdArch/></StageSection>
        <StageSection step="2" title={`Cycle & Line · ${p.name}`} sub="moved here from Products — cycle time and line assignment are line properties"><ProdCycle p={p} onNav={onNav}/></StageSection>
        ...
        <StageSection step="5" title="Sequence-Dependent Changeover" sub="the matrix, plus the solver's chosen run order and the minutes it saves"><ProdChange/></StageSection>
      </div>
    </div>
  );
}
    ...
    <div>
      ...
      <div style={{marginTop:12}}>
        <Card icon="📉" title="Derived Capacities & Bottlenecks" badge="min(stage)" info={{ what:'Each line is capped by its slowest stage.', flows:'Line cap → aggregate & production MILP.' }}
          ...
          <Reading formula="line capacity = min over stages (machines × 60 ÷ cycle × OEE)" soWhat="LINE-03 is capped by Heat-Treat — every extra unit there is worth its dual on Plan."/>
        </Card>
      </div>
    </div>
  );
...
function ProdChange() {
  ...
  const order=[0,2,1,3]; // solver's chosen sequence (indices)
  // alphabetical 0→1→2→3 vs optimized
  const seqCost=(seq)=>{ let s=0; for(let i=0;i<seq.length-1;i++){ const v=M.changeover[seq[i]][seq[i+1]]; if(typeof v==='number') s+=v; } return s; };
  const alpha=seqCost([0,1,2,3]), opt=seqCost(order), saved=(alpha-opt);
  return (
    <Grid cols={2}>
      ...
      <Card icon="⚡" title="Chosen Run Order" badge={`−${saved.toFixed(1)} hrs`} badgeTone="y" info={{ what:'The sequence the solver picks and the setup time it saves vs alphabetical.', flows:'Order → production schedule.' }}
        ...
        <SubLabel>Solver-optimized</SubLabel>
        <div style={{display:'flex', alignItems:'center', gap:4, flexWrap:'wrap'}}>
          {order.map((s,i)=><React.Fragment key={i}><span style={{fontFamily:F.mono, fontSize:10, fontWeight:700, padding:'4px 7px', border:`2px solid ${C.line}`, background:C.ac, color:C.onAc}}>{skus[s].slice(4)}</span>{i<3 && <span style={{color:C.tx3}}>→</span>}</React.Fragment>)}
          <span className="num" style={{marginLeft:'auto', fontFamily:F.disp, fontWeight:800}}>{opt.toFixed(1)} hrs</span>
        </div>
        <Reading formula="min Σ changeover(seqᵢ, seqᵢ₊₁) over all permutations"
          soWhat={`Re-ordering saves ${saved.toFixed(1)} setup hours per cycle — roughly ₹${Math.round(saved*4200).toLocaleString('en-IN')} at the setup rate.`}/>
      </Card>
    </Grid>
  );
}
