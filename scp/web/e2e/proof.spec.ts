...
test("proof: run every scenario, read a derivation, open a scenario's data in the app", async ({ page }) => {
  ...
  await expect(page.locator("table.cov tbody tr")).toHaveCount(8);

  await page.getByRole("button", { name: /Run all/ }).click();
  await expect(page.locator("table.cov .badge.ok")).toHaveCount(8, { timeout: 60_000 });
  ...
  await expect(page.getByText("What it caught (4)")).toBeVisible();
