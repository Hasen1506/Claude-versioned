...
8. **G-RF1 ✅ DONE (2026-06-07)** — full identity panel + a STATIC "panel == harness" gate. The Reference▸ConsistencyPanel DISPLAYED only **5** of the **6** cross-solver identities `golden_path.js` (HARNESS-1b) asserts — it omitted **B-13** (S&OP plan sources committed demand), and its **I-2** keyed off the *linecap* dual (`lcMax>0`) which is HONESTLY slack at TPAC volume (π=0, the SF-7 lesson) → it would have shown `•`/`info` where the harness reds/greens off the **profit-mix** binding dual. So the operator's "do the engines agree?" view silently under-reported what the build actually checks. Fix (frontend + harness, no backend): (a) **reference.jsx** — added the **B-13** row: re-derives the REAL `_loopAggregatePayload(planning)` and compares each COMMITTED SKU's `forecast` to `getItemDemand` over the same period count (the exact HARNESS-1b check); uncommitted SKUs legitimately fall back to the seed reslice so they're scoped out (a flat-seed vs seasonal-reslice compare would false-red). **Realigned I-2** to the profit-mix binding dual (`pmMax>0` ⇒ ✓, `pmMax==0` ⇒ ✗ "bottleneck claim empty", with the linecap dual now shown as *supplementary* detail labelled "all lines honestly slack, π=0"). (b) **tools/model_check.js** — new HARD check `identityParity()` (HARNESS-1's 7th): extracts the identity ID set the harness asserts (its `add('<ID> · …')` rows — the same ` · ` rule `report()` uses) and the set the panel renders (`ConsistencyPanel` `checks[].id`), and **FAILs on any symmetric difference** in EITHER direction. Both sets are parsed from the REAL source, so neither can be a stale declaration — the panel can never again silently drift from the harness. *Proven to bite:* injecting `B-13`→`B-13Z` in the panel made the gate FAIL with "harness asserts but panel omits: B-13 · panel shows but harness never asserts: B-13Z", reverted clean. *Probe (browser-boot, real loop + profit-mix; lean-with-boot-retries — `/dev/shm` is only 64M here so the Chromium renderer dies mid-boot intermittently, handled with retries):* the panel's OWN derivations go green end-to-end — **B-13: 6 committed SKUs · 0 mismatches → ok**; **I-2: profit-mix dual ₹114/u binds [Min prod TPA-9904, Min prod TPA-5540, Shared Capacity] → ok** (matches HARNESS-1b's own B-13/I-2 rows exactly). Gates OBS-3 clean · **HARNESS-1 7/7** (452 names; the parity gate is the new one) · HARNESS-1b 9/9.
