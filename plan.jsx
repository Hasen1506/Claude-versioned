...
function StagePlan({ onNav }) {
  ...
  const tabs = [
    { id:'strategy', n:'a', label:'Strategy' },
    { id:'capacity', n:'b', label:'Capacity' },
    { id:'workforce',n:'c', label:'Workforce' },
    { id:'disagg',   n:'d', label:'Disaggregation' },
  ];
  return (
    <div>
      <StageHeader n="04" title="Plan · Sales & Operations" kicker="Level-vs-chase strategy · seasonal prebuild · workforce plan · capacity duals · SKU disaggregation"
        right={<Btn kind="accent" onClick={()=>onNav('console')}>⚡ Solve Aggregate</Btn>}/>
      <SubTabNav tabs={tabs} active={sub} onChange={setSub}/>
      <div style={{padding:14}}>
        {sub==='strategy'  && <PlanStrategy/>}
        {sub==='capacity'  && <PlanCapacity/>}
        {sub==='workforce' && <PlanWorkforce/>}
        {sub==='disagg'    && <PlanDisagg/>}
      </div>
    </div>
  );
}
  ...
  return (
    <Grid cols={2}>
      ...
      <Card icon="🔑" title="Capacity Shadow Prices" badge="duals" badgeTone="y" info={{ what:'Marginal value of one more unit of each binding resource.', flows:'Duals → capital budget solver (where to invest).' }} span={2}
        dev={{ comp:'ShadowPriceCard', props:'state.aggregatePlan.shadow', note:'Consumed by Capital solver (7B).' }}>
